...
cpm.file = as.character(param.table$Value[param.table$Parameter == "fpkm_file_peak"])
...
normalized.table = normalized.table[normalized.table$chr != "chrM",]
...
write.table(quantiles, "merged_peaks_log2_FPKM_quantiles.txt",sep="\t", quote=F, row.names=F)
...
for (group in plot.groups){
	...
	pca.text.file = paste("merged_peaks_FPKM_",group,"_pca_values.txt",sep="")
	...
	pca.file = paste("merged_peaks_FPKM_pca_by_",group,".png",sep="")
	png(file=pca.file)
	...
	box.file = paste("merged_peaks_FPKM_box_plot_by_",group,".png",sep="")
	png(file=box.file)
	boxplot(temp.mat, col=labelColors, xaxt='n', main = group)
	legend("topright",legend=groups,col=color.palette,  pch=19)
	dev.off()

	cluster.file = paste("merged_peaks_FPKM_cluster_by_",group,".png",sep="")
	...
	hist.file = paste("merged_peaks_FPKM_density_by_",group,".png",sep="")
	png(file = hist.file)
