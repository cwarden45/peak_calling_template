...
region.length.kb = as.numeric(counts.table$region.length.kb)
...
if(rep.check == 1){
	...
	if (pvalue.method == "edgeR"){
		...
		if ((length(deg.groups) == 2)&(interaction.flag == "filter-overlap")){
			...
			y = DGEList(counts=prim.counts, genes=regionID)
			png(paste("merged_peaks_",comp.name,"prim_voom_plot.png",sep="_"))
			design = model.matrix(~prim.deg.grp)
...
if (interaction.flag == "no"){
	if (fdr.method == "BH"){
		...
		fdr = qobj$qvalue
		png(paste("merged_peaks_",comp.name,"_",pvalue.method,"_qvalue_plot.png",sep=""))
		qHist = hist(qobj)
		print(qHist)
		dev.off()
	...
	if(interaction.flag == "model"){
		...
		if (fdr.method == "BH"){
			...
			fdr = qobj$qvalue
			png(paste("merged_peaks_",comp.name,"_",pvalue.method,"_prim_qvalue_plot.png",sep=""))
			qHist = hist(qobj)
			print(qHist)
			dev.off()
		} else if (fdr.method == "q-lfdr"){
		...
		if (fdr.method == "BH"){
			...
			sec.fdr = qobj$qvalue
			png(paste("merged_peaks_",comp.name,"_",pvalue.method,"_sec_qvalue_plot.png",sep=""))
			qHist = hist(qobj)
			print(qHist)
			dev.off()
		} else if (fdr.method == "q-lfdr"){
...
dbr.genes = as.character(region.info$symbol[plot.flag])
dbr.genes[is.na(dbr.genes)]=""
...
if(length(dbr.genes) > 1){
	...
	if(length(plot.groups) > 1){
		...
		heatmap.file = paste("merged_peaks_",comp.name,"_",pvalue.method,"_DEG_fc_",fc.cutoff,"_fdr_",fdr.cutoff,"_pval_",pvalue.cutoff,".png",sep="")
		...
		if(interaction.flag != "no"){
			...
			heatmap.file = paste("merged_peaks_fold_change_",comp.name,"_",pvalue.method,"_DEG_fc_",fc.cutoff,"_fdr_",fdr.cutoff,"_pval_",pvalue.cutoff,".png",sep="")
			heatmap.file = gsub(":",".",heatmap.file)
			png(file = heatmap.file)
