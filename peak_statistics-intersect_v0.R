...
peak.length.dist = "peak_length_dist_MACS2_narrow.png"
peak.length.stats = "peak_length_dist_MACS2_narrow.txt"
...
#probably switch to sample.description.table$alignment.folder, if you know ahead of time you want narrow peaks
bedFiles = paste("../MACS2_Narrow_Direct_Peaks","/", longID,"/macs_",longID,peakSuffix,sep="")
			...
			if(i == 1)
				{
					...
					plot(den$x, den$y, type="l", xlab = "Peak Length", ylab = "Density",
							xlim=c(0,max.plot), ylim=c(0,0.05), col="gray")
					legend("top",legend=c("sample","merged"),col=c("gray","black"),
							lwd=2, xpd=T, inset =-0.13, ncol=2)
				}#end if(i == 1)
			else
				{
					den = density(peak.length, na.rm=T,from=0, to=max.plot)
