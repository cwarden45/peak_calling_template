normalizeTotalBinding = function (geneExpr, totalReads) {
	return(geneExpr / totalReads)
}#end def normalizeTotalExpression

avgGroupBinding = function (geneExpr, groups) {
	avg.expr = tapply(geneExpr, groups, mean)
	return(avg.expr)
}#end def avgGroupBinding
...
min.fraction.bound = as.numeric(as.character(param.table$Value[param.table$Parameter == "minimum_fraction_bound"]))
...
pvalue.method = as.character(param.table$Value[param.table$Parameter == "dba_pvalue_method"])
fdr.method = as.character(param.table$Value[param.table$Parameter == "dba_fdr_method"])
...
regionID = counts.table$regionID
region.length.kb = as.numeric(counts.table$promoter.length.kb)
...
if((aligned.type == "aligned")|(aligned.type =="quantified")){
	if(aligned.type =="aligned"){
		...
		stop("Print FPKM_norm must be either 'aligned' or 'quantified'")
	}#end else

	total.million.aligned.reads = aligned.reads / 1000000
	print(total.million.aligned.reads)

	fpk = matrix(ncol=ncol(counts), nrow=nrow(counts))
	for (i in 1:ncol(counts)){
		temp.counts = as.numeric(counts[,i])
		temp.fpk = temp.counts / region.length.kb
		fpk[,i] = temp.fpk 
	}
	FPKM = log2(round(t(apply(fpk, 1, normalizeTotalBinding, totalReads = total.million.aligned.reads)), digits=2) + min.expression)
	colnames(FPKM) = as.character(sample.label)
}else if(aligned.type == "TMM"){
	library(edgeR)
	
	colnames(counts)=sample.label
	y = DGEList(counts=counts, genes=regionID)
	y = calcNormFactors(y, method="TMM")
	
	FPKM = round(log2(rpkm(y, gene.length = 1000 * as.numeric(region.length.kb))+min.expression), digits=2)
}else{
	stop("Print FPKM_norm must be either 'aligned', 'quantified', or 'TMM'")
}#end else
...
FPKM = FPKM[bound.sample.count >= round(min.fraction.bound * ncol(counts)),]
print(dim(counts))

regionID = counts.table$regionID
region.info = counts.table[,2:8]
region.length.kb = as.numeric(counts.table$promoter.length.kb)
colnames(counts) = as.character(sample.label)
rownames(counts) = as.character(regionID)
...
average.fpkm = data.frame(t(apply(FPKM, 1, avgGroupBinding, groups = grp)))
if(length(groupIDs) == 1){
	average.fpkm = t(average.fpkm)
} else {
	average.fpkm = average.fpkm
}
colnames(average.fpkm) = paste("avg.log2.fpkm", sub("-",".",groupIDs), sep=".")
...
if (trt.group == "continuous"){
	...
	gene.cor = apply(deg.FPKM, 1, calc.gene.cor, indep.var=contrast.grp)
	
	fc.table = data.frame(cor=gene.cor)
} else {
	groupIDs = as.character(levels(as.factor(contrast.grp)))
	contrast.fpkm = data.frame(t(apply(deg.FPKM, 1, avgGroupBinding, groups = contrast.grp)))
	colnames(contrast.fpkm) = paste("avg.log2.fpkm", sub("-",".",groupIDs), sep=".")
}#end else

if((interaction.flag == "no") & (trt.group != "continuous")){
	print("Calculating fold-change for primary variable")
	trt.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",trt.group), sep=".")]
	cntl.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",groupIDs[groupIDs != trt.group]), sep=".")]
	...
	if ((trt.group == "continuous")&(trt.group2 == "continuous")){
		...
		gene.cor2 = apply(deg.FPKM, 1, calc.gene.cor, indep.var=sec.contrast.grp)
		...
		print("NOTE: 'Up-Regulated' R output refers to regions that vary with FDR and p-value cutoffs")
		...
		sec.contrast.fpkm = data.frame(t(apply(deg.FPKM, 1, avgGroupBinding, groups = sec.groupIDs)))
		colnames(sec.contrast.fpkm) = paste("avg.log2.fpkm", sub("-",".",groupIDs), sep=".")
		sec.trt.expr = sec.contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",trt.group2), sep=".")]
		sec.cntl.expr = sec.contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",sec.groups[sec.groups != trt.group2]), sep=".")]
		...
		prim.contrast.fpkm = data.frame(t(apply(deg.FPKM, 1, avgGroupBinding, groups = prim.groupIDs)))
		colnames(prim.contrast.fpkm) = paste("avg.log2.fpkm", sub("-",".",prim.groups), sep=".")
		prim.trt = trt.group
		prim.cntl = prim.groups[prim.groups != trt.group]
		prim.trt.expr = prim.contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",prim.trt), sep=".")]
		prim.cntl.expr = prim.contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",prim.cntl), sep=".")]
		...
		prim.trt.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",prim.trt), sep=".")]
		prim.cntl.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",prim.cntl), sep=".")]
		...
		sec.trt.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",sec.trt), sep=".")]
		sec.cntl.expr = contrast.fpkm[,paste("avg.log2.fpkm", sub("-",".",sec.cntl), sep=".")]
...
if(rep.check == 1){
	...
	if (pvalue.method == "edgeR"){
		...
		if ((length(deg.groups) == 2)&(interaction.flag == "filter-overlap")){
			...
			y = DGEList(counts=prim.counts, genes=regionID)
			if(aligned.type == "TMM"){
				y = calcNormFactors(y, method="TMM")
			...
			y = DGEList(counts=sec.counts, genes=regionID)
			if(aligned.type == "TMM"){
				y = calcNormFactors(y, method="TMM")
			}#end if(aligned.type == "TMM")
			y = estimateCommonDisp(y)
			...
			y = DGEList(counts=deg.counts, genes=regionID)
			if(aligned.type == "TMM"){
				y = calcNormFactors(y, method="TMM")
			}#end if(aligned.type == "TMM")
			y = estimateCommonDisp(y)
		...
		if ((length(deg.groups) == 2)&(interaction.flag == "filter-overlap")){
			...
			y = DGEList(counts=prim.counts, genes=regionID)
			png(paste("TSS_",comp.name,"prim_voom_plot.png",sep="_"))
			design = model.matrix(~prim.deg.grp)
		...
		if ((length(deg.groups) == 2)&(interaction.flag == "filter-overlap")){
			print("FPKM linear regression, Two-Step Analysis")

			if (trt.group == "continuous"){
				prim.deg.grp = as.numeric(prim.deg.grp)
			}
			
			prim.pvalue = apply(prim.FPKM, 1, gene.lm, var1=prim.deg.grp)
			...
			sec.pvalue = apply(sec.FPKM, 1, gene.lm, var1=sec.deg.grp)
		} else {
			if (length(deg.groups) == 1){
				print("FPKM linear regression with 1 variable")

				if (trt.group == "continuous"){
					var1 = as.numeric(var1)
				}
				test.pvalue = apply(deg.FPKM, 1, gene.lm, var1=var1)
			} else if ((length(deg.groups) == 2)&(interaction.flag == "no")){
				print("FPKM linear regression with 2 variables")
				...
				test.pvalue = apply(deg.FPKM, 1, gene.lm, var1=var1, var2=var2)
			} else if ((length(deg.groups) == 2)&(interaction.flag == "model")){
				print("FPKM linear regression with 2 variables plus interaction")
				...
				test.pvalue = apply(deg.FPKM, 1, gene.lm, var1=var3, var2=var1, var3=var2)
			}
		}#end else
	} else if (pvalue.method == "ANOVA"){
		if ((length(deg.groups) == 2)&(interaction.flag == "filter-overlap")){
			print("FPKM ANOVA, Two-Step Analysis")

			if (trt.group == "continuous"){
				prim.deg.grp = as.numeric(prim.deg.grp)
			}
			
			prim.pvalue = apply(prim.FPKM, 1, gene.aov, var1=prim.deg.grp)
			...
			sec.pvalue = apply(sec.FPKM, 1, gene.aov, var1=sec.deg.grp)
		} else {
			if (length(deg.groups) == 1){
				print("FPKM ANOVA with 1 variable")
				...
				print("FPKM ANOVA with 2 variables")
				...
				print("FPKM ANOVA with 2 variables plus interaction")
				var3 = as.factor(paste(var1,var2,sep=":"))
...
if (trt.group == "continuous"){
	upID = "Increased Binding"
	downID = "Decreased Binding"
} else {
	upID = paste(trt.group," Increased Binding",sep="")
	downID = paste(trt.group," Decreased Binding",sep="")	
}
...
if (interaction.flag == "no"){
	if (fdr.method == "BH"){
		...
		fdr = qobj$qvalue
		png(paste("TSS_",comp.name,"_",pvalue.method,"_qvalue_plot.png",sep=""))
		qHist = hist(qobj)
		print(qHist)
		dev.off()
	...
	if(interaction.flag == "model"){
		...
		if (fdr.method == "BH"){
			...
			fdr = qobj$qvalue
			png(paste("TSS_",comp.name,"_",pvalue.method,"_prim_qvalue_plot.png",sep=""))
			qHist = hist(qobj)
			print(qHist)
			dev.off()
		} else if (fdr.method == "q-lfdr"){
		...
		if (trt.group == "continuous"){
			pass1.status[(gene.cor >= cor.cutoff) & (prim.pvalue <= pvalue.cutoff) & (fdr <= fdr.cutoff)] = paste(trt.group," Increased Binding",sep="")
			pass1.status[(gene.cor <= -cor.cutoff) & (prim.pvalue <= pvalue.cutoff) & (fdr <= fdr.cutoff)] = paste(trt.group," Decreased Binding",sep="")
		} else{
			pass1.status[(prim.fc >= fc.cutoff) & (prim.pvalue <= pvalue.cutoff) & (fdr <= fdr.cutoff)] = paste(trt.group,"Increased Binding",sep="")
			pass1.status[(prim.fc <= -fc.cutoff) & (prim.pvalue <= pvalue.cutoff) & (fdr <= fdr.cutoff)] = paste(trt.group," Decreased Binding",sep="")
		}#end else

		print(paste("Primary Up-Regulated: ",length(pass1.status[pass1.status == paste(trt.group," Increased Binding",sep="")]),sep=""))
		print(paste("Primary Down-Regulated: ",length(pass1.status[pass1.status == paste(trt.group," Decreased Binding",sep="")]),sep=""))
		...
		if (fdr.method == "BH"){
			...
			sec.fdr = qobj$qvalue
			png(paste("TSS_",comp.name,"_",pvalue.method,"_sec_qvalue_plot.png",sep=""))
			qHist = hist(qobj)
			print(qHist)
			dev.off()
		} else if (fdr.method == "q-lfdr"){
		...
		if (trt.group2 == "continuous"){
			pass2.status[(gene.cor2 >= cor.cutoff2) & (sec.pvalue <= pvalue.cutoff2) & (sec.fdr <= fdr.cutoff2)] = paste(trt.group," Increased Binding",sep="")
			pass2.status[(gene.cor2 <= -cor.cutoff2) & (sec.pvalue <= pvalue.cutoff2) & (sec.fdr <= fdr.cutoff2)] = paste(trt.group," Decreased Binding",sep="")
		} else{
			pass2.status[(sec.fc >= fc.cutoff2) & (sec.pvalue <= pvalue.cutoff2) & (sec.fdr <= fdr.cutoff2)] = paste(trt.group," Increased Binding",sep="")
			pass2.status[(sec.fc <= -fc.cutoff2) & (sec.pvalue <= pvalue.cutoff2) & (sec.fdr <= fdr.cutoff2)] = paste(trt.group," Decreased Binding",sep="")
		}#end else

		print(paste("Secondary Up-Regulated: ",length(pass2.status[pass2.status == paste(trt.group," Increased Binding",sep="")]),sep=""))
		print(paste("Secondary Down-Regulated: ",length(pass2.status[pass2.status == paste(trt.group," Decreased Binding",sep="")]),sep=""))
			
		pvalue.table = data.frame(prim.pvalue = prim.pvalue, prim.FDR = fdr,
										sec.pvalue=sec.pvalue, sec.fdr=sec.fdr)
			
		status = rep("No Change", times=length(fdr))
		status[(pass1.status == paste(trt.group," Increased Binding",sep="")) & (pass2.status == "No Change")] = upID
		status[(pass1.status == paste(trt.group," Decreased Binding",sep="")) & (pass2.status == "No Change")] = downID
	} else{
		stop("interaction must be \"no\", \"model\", or \"filter-overlap\"")
	}#end else
}#end else
...
if(rep.check == 1){
	dbr.table = data.frame(regionID, region.info,
							average.fpkm, fc.table,
...
dbr.file = paste("TSS_",comp.name,"_",pvalue.method,"_DBR_fc_",fc.cutoff,"_fdr_",fdr.cutoff,"_pval_",pvalue.cutoff,".txt",sep="")
dbr.file = gsub(":",".",dbr.file)
write.table(dbr.table, file=dbr.file, row.names=F, quote=F, sep="\t")

final.dbr.file = paste(user.folder,"/Differential_Binding_Analysis/TSS_",comp.name,"_DBR_stats.txt",sep="")
write.table(dbr.table, file=final.dbr.file, row.names=F, quote=F, sep="\t")

plot.flag = (status != "No Change")

temp.fpkm = FPKM
temp.fpkm = temp.fpkm[plot.flag, ]
dbr.genes = as.character(region.info$gene[plot.flag])
print(length(dbr.genes))

if(length(dbr.genes) > 1){
	...
	if(length(plot.groups) > 1){
		...
		heatmap.file = paste("TSS_",comp.name,"_",pvalue.method,"_DEG_fc_",fc.cutoff,"_fdr_",fdr.cutoff,"_pval_",pvalue.cutoff,".png",sep="")
		...
		if(interaction.flag != "no"){
			...
			heatmap.file = paste("TSS_fold_change_",comp.name,"_",pvalue.method,"_DEG_fc_",fc.cutoff,"_fdr_",fdr.cutoff,"_pval_",pvalue.cutoff,".png",sep="")
			heatmap.file = gsub(":",".",heatmap.file)
			png(file = heatmap.file)
